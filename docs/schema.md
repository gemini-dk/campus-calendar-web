...
- `class_dates/{classDateId}`（旧 `timetable_class_dates`）  
  フィールド:  
  ...
  - `updatedAt` (timestamp)  
  ドキュメント ID は `classDate` か、`{classDate}_{hash}` の形式で重複を防ぎます。

### インデックス例

- `/users/{uid}/academic_years/{year}/timetable_classes`: `classType` + `termNames`（array-contains）複合。
- `/users/{uid}/academic_years/{year}/timetable_classes/{classId}/class_dates`: `classDate` 昇順、`isExcludedFromSummary` フィルタ。
...
| `timetable_class_dates` | `/users/{uid}/academic_years/{fiscalYear}/timetable_classes/{classId}/class_dates/{classDateId}` |
...
| 出欠集計 | `/users/{uid}/academic_years/{year}/timetable_classes/{classId}/class_dates`: `isExcludedFromSummary == false`, `classDate` ASC |
