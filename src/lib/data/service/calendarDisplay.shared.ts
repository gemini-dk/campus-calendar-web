...
export type AccentColor = "default" | "holiday" | "saturday";
export type BackgroundColorType = "none" | "sunday" | "holiday" | "exam" | "reserve";
...
export function computeCalendarDisplayInfo(
  dateId: string,
  day: CalendarDay | null,
  terms: CalendarTerm[],
...
export function normalizeCalendarDateId(value: string | undefined | null): string | null {
  ...
  const digits = trimmed.replaceAll("-", "").replaceAll("/", "").replaceAll(".", "");
  if (/^\d{8}$/.test(digits)) {
...
function resolveTermForDay(day: CalendarDay | null, terms: CalendarTerm[]): CalendarTerm | null {
  if (!day) {
    return null;
  }
...
function computeAcademicDisplay(
  ...
  const backgroundColor = determineBackgroundColor(normalizedType, actualWeekday, hasSaturdayClasses);
  const resolvedWeekdayNumber = resolveClassWeekday(day, actualWeekday);
  const suppressClassDetails = shouldSuppressClassDetails(normalizedType, actualWeekday, hasSaturdayClasses);
  ...
  const classOrder = suppressClassDetails || typeof day.classOrder !== "number" ? null : day.classOrder;
...
function normalizeDayType(type?: string | null): NormalizedDayType {
  ...
  if (
    ["holiday", "休講日", "休講", "cancelled", "canceled", "休講日特別"].some((value) => lowered.includes(value))
  ) {
    return "holiday";
  }
  if (["exam", "試験", "試験日", "test"].some((value) => lowered.includes(value))) {
    return "exam";
  }
  if (["reserve", "予備日", "makeup", "補講予備"].some((value) => lowered.includes(value))) {
    return "reserve";
  }
  return "other";
}
...
function computeAcademicLabel({
  ...
  const normalizedClassOrder =
    typeof classOrder === "number" && Number.isFinite(classOrder) && classOrder > 0 ? Math.trunc(classOrder) : null;
...
function computeAcademicSubLabel({
  ...
  const description = typeof day.description === "string" && day.description.length > 0 ? day.description : undefined;
