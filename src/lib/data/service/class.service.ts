...
export type CreateClassDateScheduleInput = {
  userId: string;
  fiscalYear: string;
  classId: string;
  classDate: string;
...
export async function createClassDateSchedule({
  userId,
  fiscalYear,
  classId,
  classDate,
  periods,
}: CreateClassDateScheduleInput): Promise<void> {
  const trimmedUserId = userId.trim();
  if (!trimmedUserId) {
  ...
  const rawType = typeof classData.classType === 'string' ? classData.classType.trim() : 'in_person';
  const normalizedClassType: CreateTimetableClassParams['classType'] =
    rawType === 'online' || rawType === 'hybrid' || rawType === 'on_demand' || rawType === 'in_person'
      ? rawType
      : 'in_person';

  const classDatesCollection = collection(classRef, 'class_dates');
  const classDateId = buildClassDateId(classRef.id, normalizedDate, normalizedPeriods);
  const classDateRef = doc(classDatesCollection, classDateId);
  ...
  await setDoc(classDateRef, {
    ...
    isCancelled: false,
    deliveryType: buildDeliveryType(normalizedClassType),
